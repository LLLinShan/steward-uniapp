  ...
  <view class="page">
    <scroll-view class="page-list" scroll-y>
      ...
      <div class="page-button">
        <u-button type="default" :ripple="true" @click="edit(detailInfo.address)">编辑</u-button>
        <u-button type="primary" :ripple="true" :loading="loading" @click="goCardDetail(detailInfo)"
          >门禁卡管理</u-button
        >
        <u-button
          type="error"
          v-if="detailInfo.blockStatus === '1'"
          :ripple="true"
          @click="onChangeBlock(0, '启用')"
          >启用门禁</u-button
        >
        <u-button
          type="error"
          v-if="detailInfo.blockStatus === '0'"
          :ripple="true"
          @click="onChangeBlock(1, '禁用')"
          >禁用门禁</u-button
        >
      </div>
    </scroll-view>
...
export default {
  ...
  methods: {
    ...
    goCardDetail(item) {
      ...
      uni.navigateTo({
        url: `/pages/home/card/detail?memberId=${item.memberId}&houseId=${item.houseId}`
      })
    },
    // 删除人员事件
    onDel() {
