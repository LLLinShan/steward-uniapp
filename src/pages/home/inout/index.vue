<template>
  <view class="page">
    ...
    <view class="search-popup">
      <u-popup v-model="show" mode="top" border-radius="0" width="100%" height="650rpx">
        <u-form ref="form" :error-type="['toast']" label-width="220" :model="params">
          ...
          <div class="search-title">出入时间</div>
          <u-form-item>
            <u-input
              placeholder="请选择出入时间"
              type="select"
          ...
          <div class="search-title">出入方向</div>
          <div class="search-li">
            <ul>
              <li
...
export default {
  ...
  methods: {
    getDic() {
      let list = Utils.getDicList('direction')
      this.directionList = list.map(item => ({
        ...item,
        select: true
      }))
    },
    ...
    async getList() {
      ...
      const { result, data } = await this.$request.listEntranceLimit({
        ...this.page,
        params: {
    ...
    // 确定
    confirmBtn() {
      this.params.direction = this.directionList
        .filter(item => item.select)
        .map(item => {
          return Number(item.value)
        })
      // this.personName = this.personNameMore
      this.show = false
      this.page.pageNum = 1
      this.list = []
      this.getList()
