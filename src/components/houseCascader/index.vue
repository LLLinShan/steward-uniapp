...
export default {
  ...
  methods: {
     ...
     */
    async getData(id) {
      const { result, data } = await this.$request.getHouseTreeNodes({
        apiLoading: true,
        structId: id
      })
      if (result !== '1') {
     ...
     */
    onNext({ structId, type, label, memberId }) {
      ...
      if (this.hasPerson) {
        ...
        let address = this.list.map(item => item.label)
        address.push(label)
        this.$emit('getHouse', {
          structId,
